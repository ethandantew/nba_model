...
def get_rosters(cur_season):
    ...
    team_df_merged = team_df.merge(
        roster_df, how="left", left_on="id", right_on="TeamID"
    )
    rosters = team_df_merged[
        [
            "abbreviation",
            "TeamID",
            "PLAYER_ID",
...
def pace_calculator(FGA, FTA, OREB, OPPDREB, FG, TOV, MIN):
    return (
        48
        * (FGA + 0.4 * FTA - 1.07 * (FGA - FG) * (OREB / (OREB + OPPDREB)) + TOV)
        / MIN
    )
...
def get_todays_games_player(game_date, rosters):
    ...
    games_df["HOME_MATCHUP"] = (
        games_df["HOME_TEAM_ABBREVIATION"]
        + " vs. "
        + games_df["AWAY_TEAM_ABBREVIATION"]
    )
    games_df["AWAY_MATCHUP"] = (
        games_df["AWAY_TEAM_ABBREVIATION"] + " @ " + games_df["HOME_TEAM_ABBREVIATION"]
    )
    home_teams = games_df[
        [
            "GAME_ID",
    ...
    away_teams.rename(
        columns={
            ...
            "AWAY_MATCHUP": "MATCHUP",
        },
        inplace=True,
    )
    team_df = pd.concat([home_teams, away_teams], ignore_index=True)
    roster_merged = team_df.merge(
        rosters, how="left", left_on="TEAM_ID", right_on="TeamID"
    )
...
def get_player_data(cur_date):
    ...
    game_logs = all_game_logs.merge(
        rosters, how="left", left_on="PLAYER_ID", right_on="PLAYER_ID"
    )
    game_logs = game_logs[
        [
            "SEASON_YEAR",
    ...
    ].rename(index=str, columns={"PLAYER_NAME_x": "PLAYER_NAME"})
    game_logs = game_logs.sort_values(by=["PLAYER_ID", "GAME_DATE"]).set_index(
        ["GAME_ID"]
    )
    player_games_cleaned = game_logs[
        [
            "PLAYER_ID",
            "PLAYER_NAME",
            "POSITION",
    ...
    merged_games_df = player_games_cleaned.merge(
        todays_games, how="outer", on="GAME_ID"
    )
    columns_to_fill_na = [
        "PLAYER_ID_x",
        "PLAYER_NAME_x",
    ...
    for column in columns_to_fill_na:
        merged_games_df[column] = merged_games_df[column].fillna(
            merged_games_df[column.replace("_x", "_y")]
        )
    # drop columns that were created from the merge and rename the columns that were kept
    merged_games_df = merged_games_df.drop(
        [
            "PLAYER_ID_y",
            "PLAYER_NAME_y",
            "POSITION_y",
    ...
    merged_games_df = merged_games_df.rename(
        columns={
    ...
    for game_date in missing_game_dates:
        ...
        passing_stats_list.append(player_passing_stats)
    player_passing_stats_df = pd.concat(passing_stats_list)
    player_passing_stats_df = player_passing_stats_df.sort_values(
        ["PLAYER_ID", "GAME_DATE"]
    )
    player_passing_stats_df["POTENTIAL_AST_L5"] = player_passing_stats_df.groupby(
        "PLAYER_ID"
    )["POTENTIAL_AST"].transform(lambda x: x.shift().rolling(5, min_periods=5).mean())
    ...
    team_games = team_gamelogs.sort_values(["TEAM_ID", "GAME_DATE"]).set_index(
        ["GAME_ID"], drop=False
    )
    team_games["VS_TEAM_ID"] = team_games.loc[:, ("MATCHUP")].str[-3:]
    team_games = team_games[
        team_games.columns.drop(list(team_games.filter(regex="RANK")))
    ]
    ...
    for column in columns_to_fill_na:
        team_games_merged[column] = team_games_merged[column].fillna(
            team_games_merged[column.replace("_x", "_y")]
        )
    # drop columns that were created from the merge and rename the columns that were kept
    team_games_merged = team_games_merged.drop(
        ["TEAM_ID_y", "TEAM_ABBREVIATION_y", "GAME_DATE_y", "MATCHUP_y"], axis=1
    )
    team_games_merged = team_games_merged.rename(
        columns={
            "TEAM_ID_x": "TEAM_ID",
            "TEAM_ABBREVIATION_x": "TEAM_ABBREVIATION",
            "GAME_DATE_x": "GAME_DATE",
            "MATCHUP_x": "MATCHUP",
        }
    )
    ...
    team_games = team_games.sort_values(["TEAM_ID", "GAME_DATE"]).set_index(
        ["GAME_ID"], drop=False
    )
    ...
    own_team_games["PREV_GAME_DATE"] = own_team_games.groupby("TEAM_ABBREVIATION")[
        "GAME_DATE"
    ].shift(1)
    own_team_games["DAYS_SINCE_LAST_GAME"] = (
        pd.to_datetime(own_team_games["GAME_DATE"])
        - pd.to_datetime(own_team_games["PREV_GAME_DATE"])
    ).dt.days
    own_team_games["B2B_FLAG"] = [
        1 if days == 1 else 0 for days in own_team_games["DAYS_SINCE_LAST_GAME"]
    ]
    own_team_games["HOME_FLAG"] = [
        1 if "vs." in matchup else 0 for matchup in team_games["MATCHUP"]
    ]
    own_team_games = own_team_games.drop(
        ["PREV_GAME_DATE", "DAYS_SINCE_LAST_GAME"], axis=1
    )
    ...
    windowed_sums = (
        pd.DataFrame(
            teams_grouped.rolling(center=False, window=10, win_type="triang")[pace_lis]
            .mean()
            .shift()
        )
        .rename(index=str, columns=team_dict)
        .reset_index()
    )
    # We can apply our pace_calculator helper now for L10 games
    windowed_sums["PACE_L10"] = pace_calculator(
        FGA=windowed_sums["TEAM_FGA_AVGLAST10GAMES"],
        FTA=windowed_sums["TEAM_FTA_AVGLAST10GAMES"],
        OREB=windowed_sums["TEAM_OREB_AVGLAST10GAMES"],
        OPPDREB=windowed_sums["VS_TEAM_DREB_AVGLAST10GAMES"],
        FG=windowed_sums["TEAM_FGM_AVGLAST10GAMES"],
        TOV=windowed_sums["TEAM_TOV_AVGLAST10GAMES"],
        MIN=windowed_sums["TEAM_MIN_AVGLAST10GAMES"],
    )
    # now join back to the original dataframe and drop the columns we don't need
    merged_tg = merged_tg.merge(
        windowed_sums,
        how="inner",
        left_on=["TEAM_ABBREVIATION_x", "GAME_ID"],
        right_on=["TEAM_ABBREVIATION_x", "GAME_ID"],
    )
    merged_tg = merged_tg.drop(
        [
            "TEAM_FGA_AVGLAST10GAMES",
            "TEAM_FTA_AVGLAST10GAMES",
            "TEAM_OREB_AVGLAST10GAMES",
            "VS_TEAM_DREB_AVGLAST10GAMES",
            "TEAM_FGM_AVGLAST10GAMES",
            "TEAM_TOV_AVGLAST10GAMES",
            "TEAM_MIN_AVGLAST10GAMES",
        ],
        axis=1,
    ).set_index(["GAME_ID"], drop=True)
    ...
    expander = (
        merged_tg.groupby("TEAM_ABBREVIATION_x")
        .PACE_L10.expanding(min_periods=2)
        .mean()
        .reset_index()
    )
    ...
    final_team_df = pd.merge(
        ...
        right=expander,
        how="inner",
        left_on=["TEAM_ABBREVIATION_y", "GAME_ID"],
        right_on=["TEAM_ABBREVIATION_x", "GAME_ID"],
    ).rename(
        index=str,
        columns={
            "PACE_L10_x": "GAME_PACE",
            "PACE_L10_y": "OWN_TEAM_PACE_L10",
            "PTS_x": "PTS_ACTUAL",
            "PACE_L10": "VS_TEAM_PACE_L10",
        },
    )
    final_team_df = final_team_df[
        final_team_df.columns.drop(list(final_team_df.filter(regex="RANK")))
    ...
    merged_player_data = pd.merge(
        ...
        final_team_df,
        how="inner",
        left_on=["TEAM", "GAME_ID"],
        right_on=["TEAM_ABBREVIATION_x_x", "GAME_ID"],
    ).rename(index=str, columns={"GAME_DATE_x": "GAME_DATE"})
    ...
    merged_player_data = merged_player_data.sort_values(
        ["PLAYER_ID", "GAME_DATE"]
    ).rename(
        index=str,
        columns={
            "TEAM_ABBREVIATION_y": "VS_TEAM_ABBREVIATION",
            "PTS_x": "PTS_ACTUAL",
            "WL_x": "WL",
        },
    )
    merged_player_data = merged_player_data.drop(
        ["GAME_DATE_y", "TEAM_ABBREVIATION_x_x", "TEAM_ABBREVIATION_x_y", "GAME_PACE"],
        axis=1,
    ).reset_index(drop=True)
    merged_player_data["PACE"] = pace_calculator(
        FGA=merged_player_data["TEAM_FGA"],
        FTA=merged_player_data["TEAM_FTA"],
        OREB=merged_player_data["TEAM_OREB"],
        OPPDREB=merged_player_data["VS_TEAM_DREB"],
        FG=merged_player_data["TEAM_FGM"],
        TOV=merged_player_data["TEAM_TOV"],
        MIN=merged_player_data["TEAM_MIN"],
    )
    ...
    temp_merged_player_df["GAME_DATE"] = pd.to_datetime(
        temp_merged_player_df["GAME_DATE"]
    )
    temp_merged_player_df.sort_values(
        ["VS_TEAM_ABBREVIATION", "POSITION", "GAME_DATE"], inplace=True
    )
    summed_by_position_date = temp_merged_player_df.groupby(
        ["GAME_DATE", "VS_TEAM_ABBREVIATION", "POSITION"]
    )[feature_list].sum()
    columns_to_divide = [
        col for col in summed_by_position_date.columns if col not in ["MIN"]
    ]
    summed_by_position_date[columns_to_divide] = summed_by_position_date[
        columns_to_divide
    ].div(summed_by_position_date["MIN"], axis=0)
    summed_by_position_date[columns_to_divide] = summed_by_position_date[
        columns_to_divide
    ]
    ...
    for feature in feature_list:
        ...
        summed_by_position_date[feature_avg_last_10] = summed_by_position_date.groupby(
            ["VS_TEAM_ABBREVIATION", "POSITION"]
        )[feature].transform(
            lambda x: x.rolling(window=10, min_periods=3).mean().shift()
        )
    ...
    summed_by_position_date = summed_by_position_date[
        ["GAME_DATE", "VS_TEAM_ABBREVIATION", "POSITION"] + feature_name_list
    ]
    summed_by_position_date = summed_by_position_date.rename(
        columns={"VS_MIN_AVGPM_LAST10G": "VS_MIN_AVG_LAST10G"}
    )
    merged_player_data = pd.merge(
        merged_player_data,
        summed_by_position_date,
        how="left",
        left_on=["GAME_DATE", "VS_TEAM_ABBREVIATION", "POSITION"],
        right_on=["GAME_DATE", "VS_TEAM_ABBREVIATION", "POSITION"],
    )
    merged_player_data["REB"] = merged_player_data["OREB"] + merged_player_data["DREB"]
    merged_player_data = merged_player_data.drop(
        ["MATCHUP_x", "VS_TEAM_ID"], axis=1
    ).reset_index(drop=True)
...
def get_model_data(cur_date):
    ...
    for window_size in window_sizes:
        ...
        player_games_mean = (
            pd.DataFrame(
                player_games_grouped.rolling(
                    center=False, window=window_size, win_type="triang"
                )[feature_list]
                .mean()
                .shift()
            )
            .rename(index=str, columns=dictionary_avg)
            .reset_index()
        )
        player_games_stdv = (
            pd.DataFrame(
            ...
            .rename(index=str, columns=dictionary_std)
            .reset_index()
        )
    ...
    player_data_cleaned_mod = (
        player_data_mod.set_index(["PLAYER_ID", "GAME_ID"])
    ...
    for column_name in player_data_cleaned_mod.columns:
        if "_AVGPM_" in column_name:
            player_data_cleaned_mod[column_name] = (
                player_data_cleaned_mod[column_name] * 36
            )
