...
def numFire():
    ...
    nfdf = pd.DataFrame(list(zip(players, pmins, pts, rebs, asts, tpm)), columns=['Player', 'NF_Mins', 'NF_Pts', 'NF_Rebs', 'NF_Asts', 'NF_3PM'])
    return(nfdf)
...
def main():
    ...
    merged_data = pd.merge(numberfire_data, sportsline_data, how='outer', left_on='Player', right_on='PLAYER')
    df = merged_data
    ...
    df = df.dropna()
    cols = ['Player', 'POS', 'TEAM', 'Avg_mins', 'Avg_pts', 'Avg_rebs', 'Avg_asts', 'NF_3PM', 'Mins_diff', 'Pts_diff', 'Rebs_diff', 'Asts_diff']
    df = df[cols]
    colnames = ['Player', 'Position', 'Team', 'Min', 'Pts', 'Rebs', 'Asts', 'P3ms', 'Mins_diff', 'Pts_diff', 'Rebs_diff', 'Asts_diff']
    df.columns = colnames
    return(df)
